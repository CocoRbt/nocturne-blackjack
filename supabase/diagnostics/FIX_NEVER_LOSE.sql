...
-- 1) Les all-in perdues restent perdues (plus de recollage sur le record).
-- 2) Les scores glitchés (> 2 M crédits, peu de parties) reviennent à 100.

-- dès qu’on tombait à ~0 (all-in), donc le crédit ne faisait que monter.

-- Remet les stacks glitchés (nouveau joueur à des millions) au départ.
...
set
  ...
  vault = 0,
  updated_at = now()
where balance >= 200000000
  and coalesce(games_played, 0) < 500;
