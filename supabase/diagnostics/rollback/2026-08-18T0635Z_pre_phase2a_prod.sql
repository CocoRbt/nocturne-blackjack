-- NOCTURNE — Sauvegarde rollback pré-Phase 2a (production)
-- Date UTC : 2026-08-18T06:35:00Z
-- Source : définitions reconstruites depuis les migrations repo appliquées en prod
--          jusqu'à 20260817200000_losses_stick.sql (+ ensure_circle_membership #171920).
-- NOTE : pg_get_functiondef live non capturé (agent sans accès DB prod).
--        Vérifier PITR / backup Supabase dashboard avant rollback.
-- Rollback : exécuter ce fichier en une transaction dans le SQL Editor prod.

-- === enforce_score_invariants + trigger (migration #172000) ===
...
-- === sync_my_score (migration #172000) ===
...
-- === ensure_circle_membership (migration #171920, inchangé par #172000) ===
...
begin
  ...
  if v_other.id is not null then
    ...
    if v_old.profile_id is not null then
      ...
      on conflict (profile_id) do update set
        balance = greatest(s.balance, excluded.balance),
        peak_balance = greatest(s.peak_balance, excluded.peak_balance),
        vault = greatest(coalesce(s.vault, 0), excluded.vault),
        hands_played = greatest(s.hands_played, excluded.hands_played),
        blackjacks = greatest(s.blackjacks, excluded.blackjacks),
        best_streak = greatest(s.best_streak, excluded.best_streak),
        highest_table = excluded.highest_table,
  ...
  -- Si la fiche est vide mais le pic du pseudo est millionnaire, recoller
  update public.player_scores s
  set
    balance = greatest(s.balance, s.peak_balance - coalesce(s.vault, 0)),
    updated_at = now()
  where s.profile_id = v_uid
    and (s.balance + coalesce(s.vault, 0)) < 100
    and s.peak_balance >= 100000000;
...
-- === Triggers attendus sur public.player_scores (état pré-Phase 2a) ===
-- trg_player_scores_invariants → enforce_score_invariants()  (BEFORE INSERT OR UPDATE)
-- trg_player_scores_wealth_peak → enforce_wealth_peak()       (BEFORE INSERT OR UPDATE, migration #171800)
-- Vérifier en prod :
-- SELECT tgname, pg_get_triggerdef(oid) FROM pg_trigger
-- WHERE tgrelid = 'public.player_scores'::regclass AND NOT tgisinternal ORDER BY tgname;
