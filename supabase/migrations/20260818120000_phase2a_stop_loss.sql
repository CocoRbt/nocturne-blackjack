-- Phase 2a stop-loss (nouvelle migration, ne rejoue pas #172000).
-- Objectif : une baisse réelle de balance n’est plus annulée par la sync.
-- AUCUN UPDATE de player_scores / wallets (aucune restauration joueur).

-- A) Trigger : record monotone uniquement. Jamais OLD.balance / OLD.vault.
--    Jamais peak_balance → balance ni peak_balance → vault.
...
-- B) sync_my_score : retire « même games_played + balance ↓ → restaurer OLD ».
--    Conservé : push stale (games_played inférieur), cohérence coffre,
--    rejet d’un gros gain sans nouvelle partie (anti-stale, pas anti-perte).
...
begin
  ...
  if v_prev.profile_id is not null then
    -- Stale : un appareil en retard (moins de parties) ne peut pas écraser.
    ...
    if not v_stale then
      ...
      -- Coffre ↑ sans débit solde correspondant (hors transfert patrimoine stable).
      ...
      -- Coffre ↓ sans crédit solde correspondant : on ignore le coffre poussé.
      if v_vault_delta < 0 and abs(v_bal_delta - (-v_vault_delta)) > 1 then
        v_vault := coalesce(v_prev.vault, 0);
        v_vault_delta := 0;
        v_bal_delta := v_bal - coalesce(v_prev.balance, 0);
      ...
      -- Phase 2a : PAS de restauration si games_played identique et balance ↓.
      -- Une mise débitée avant games+1 doit rester une baisse réelle.

      -- Gros gain sans nouvelle partie : refill minuscule / défi OK, le reste rejeté.
      if v_bal_delta > 0
        and v_vault_delta >= 0
...
-- C) ensure_circle_membership : retirer le recollage millionnaire peak → balance.
