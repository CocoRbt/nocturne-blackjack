-- Wipe millionnaire : record ≥ 1M crédits mais solde ~0 (nouvelle session
-- / hydrate d’une fiche vide). Recolle le jouable sur le pic, et refuse
-- d’écrire 0 par-dessus un record.

-- 1) Répare les fiches déjà à 0 (Kikiloki et tout autre cas identique)
...
begin
  ...
  -- Millionnaire à 0 = wipe, recoller le jouable sur le pic
  if (coalesce(v_bal, 0) + coalesce(v_vault, 0)) < 100 and v_peak >= 100000000 then
    v_bal := greatest(v_bal, v_peak - coalesce(v_vault, 0));
  end if;
...
-- Reclaim : copier l’ancien wallet comme PLANCHER (jamais une fiche à 0)
