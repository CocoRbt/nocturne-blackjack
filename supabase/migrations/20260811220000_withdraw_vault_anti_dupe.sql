-- Retrait coffre → solde atomique + anti-duplication sync.
-- Bug : après un cadeau, un retrait local puis un pull stale (Math.max coffre)
-- recréait le coffre tout en gardant le solde augmenté.
...
-- Durcit sync : solde ↑ + coffre non ↓ = création de richesse refusée
-- (sauf refill base / petits bonus défis), même si le client re-pousse un coffre stale.
